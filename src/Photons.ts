...
export default class Photons {
  ...
  createPhotonPropagator(yDirMeansDown = true): Operator {
    ...
    return Operator.add([
      Operator.outer([
        Operator.shift(dimX, +1),
        Operator.identity([dimY]),
        Operator.indicator([dir], [">"])
      ]),
      Operator.outer([
        Operator.shift(dimX, -1),
        Operator.identity([dimY]),
      ...
      Operator.outer([
        Operator.identity([dimX]),
        Operator.shift(dimY, +s),
        Operator.indicator([dir], ["v"])
      ]),
      Operator.outer([
        Operator.identity([dimX]),
        Operator.shift(dimY, -s),
  ...
  propagatePhotons(yDirMeansDown = true): void {
    ...
    _.range(this.nPhotons).forEach((i: number) => {
      this.vector = photonPropagator.mulVecPartial(
        this.vectorPosDirIndicesForParticle(i),
        this.vector
      );
    });
  }
  ...
  measureAbsorptionAtOperator(
    ...
    const localizedId = Operator.indicator(
      [this.dimX, this.dimY],
      [`${posX}`, `${posY}`]
    );
    const newVector = localizedOperator.mulVecPartial(
      this.vectorIndicesForParticle(photonId),
      this.vector
    );
    const oldVector = localizedId.mulVecPartial(
      this.vectorPosIndicesForParticle(photonId),
      this.vector
    );
  ...
  vectorValuedMeasurement(
    ...
    return basis
      .map(coordStr => {
        ...
        const vectorProjected = projection.mulVecPartial(
          dirPolInd,
          oldVectorHere
        );
        const inputProjectedProbabability = vectorProjected.normSquared();
        const allId = this.vectorIndicesForParticle(photonId);
        const outputProjectedProbability = localizedOperator
          .mulVecPartial([...allId], vectorProjected)
          .normSquared();
        ...
        newPhotons.vector = proj
          .mulVecPartial([...allId], this.vector)
          ._removeDimension([...allId]);
        return {
          photonId: photonId,
          x: posX,
          y: posY,
   ...
   */
  createSinglePhotonInteraction(
    opsWithPos: [number, number, Operator][]
  ): Operator {
    //
    const localizedOpsShifted = opsWithPos.map(
      (x: [number, number, Operator]) => {
        const [posX, posY, op] = x;
        const shiftedOp = op.sub(
          Operator.identity([Dimension.direction(), Dimension.polarization()])
        );
  ...
  actOnSinglePhotons(opsWithPos: [number, number, Operator][]): void {
    const singlePhotonInteraction = this.createSinglePhotonInteraction(
      opsWithPos
    );
    _.range(this.nPhotons).forEach((i: number) => {
      this.vector = singlePhotonInteraction.mulVecPartial(
        this.vectorIndicesForParticle(i),
        this.vector
      );
    });
  }
    ...
    const aggregated = _.chain(this.vector.entries)
      ...
      .map((entries: any) => {
        ...
        const probability = entries
          .map((entry: any) => entry.value.abs2())
